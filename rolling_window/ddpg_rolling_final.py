...
# 💥 修改点 2：改为 setup_ddpg_args，并配置 DDPG
def setup_ddpg_args(env_args, cwd_path):
    ...
    # 💡 DDPG 特有：设置探索噪声 (SAC 不需要这个，因为它是随机策略)
...
def real_test_inference(test_df, stock_dim, indicators, args, init_capital, init_shares):
    ...
    # 💥 修改点 3：推理阶段使用 AgentDDPG 加载权重
    agent = AgentDDPG(args.net_dims, args.state_dim, args.action_dim)
    agent.save_or_load_agent(args.cwd, if_save=False)
    agent.act.eval()  # 开启 eval 模式，关闭 DDPG 的探索噪声，直接输出确定性动作
...
if __name__ == '__main__':
    ...
    # 💥 修改点 4：修改 checkpoint 保存目录，防止覆盖 SAC 权重
    global_cwd = "./checkpoints/ddpg_continuous_learning"
    ...
    for i in range(TRAIN_WINDOW + VAL_WINDOW, len(unique_dates) - TEST_WINDOW, TEST_WINDOW):
        ...
        try:
            # 💥 修改点 5：调用 setup_ddpg_args
            args = setup_ddpg_args(env_params, global_cwd)
            ...
            print("[-->] Starting DDPG Agent Training (Continual Learning)...")
            train_agent(args)
    ...
    if all_test_results:
        # 💥 修改点 6：修改最终结果保存的文件名
        pd.concat(all_test_results).reset_index(drop=True).to_csv("ddpg_rolling_results_v3_fixed.csv", index=False)
        print(f"\n[SUCCESS] Backtest V3 Fixed complete! File saved as ddpg_rolling_results_v3_fixed.csv")
