...
def real_test_inference(test_df, stock_dim, indicators, args, init_capital, init_shares):
    ...
    while not done:
        ...
        # 2. 真实持股数 (根据 FinRL 源码，紧跟在价格数组后面)
        true_shares = np.array(env.env.state[1 + stock_dim: 1 + stock_dim * 2])
        # 3. 真实价格！绝对不信环境 state，直接查外部 DataFrame 源数据
...
if __name__ == '__main__':
    ...
    global_cwd = "./checkpoints/sac_continuous_learning"
    ...
    for i in range(TRAIN_WINDOW + VAL_WINDOW, len(unique_dates) - TEST_WINDOW, TEST_WINDOW):
        ...
        try:
            args = setup_sac_args(env_params, global_cwd)
            ...
            print("[-->] Starting SAC Agent Training (Continual Learning)...")
            train_agent(args)
    ...
    if all_test_results:
        # 保存新的结果
        pd.concat(all_test_results).reset_index(drop=True).to_csv("sac_rolling_results_v3_fixed.csv", index=False)
        print(f"\n[SUCCESS] Backtest V3 Fixed complete! File saved as sac_rolling_results_v3_fixed.csv")
